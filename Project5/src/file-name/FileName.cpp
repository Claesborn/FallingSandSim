...
int main() {
    ...
    // SELECTION SORT
    // We'll sort from the right side (largest elements to the
    // end). i goes from the last index down to 1 (or up to
    // numValues-1 if you prefer).
    for (unsigned int i = numValues - 1; i > 0; --i) {
        ...
        // Swap the largest found element with the element at index
        // i
        if (maxIndex != i) {
...
void drawBars(sf::RenderWindow&       window,
              const std::vector<int>& data) {
    float barWidth =
        static_cast<float>(window.getSize().x) / data.size();

    for (unsigned int i = 0; i < data.size(); ++i) {
        sf::RectangleShape bar;
        bar.setSize(
            sf::Vector2f(barWidth, static_cast<float>(data[i])));
