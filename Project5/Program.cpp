...
LRESULT CALLBACK CustomWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if (g_pProgram)
    {
        return g_pProgram->HandleCustomWindowProc(hwnd, uMsg, wParam, lParam);
    }
...
LRESULT Program::HandleCustomWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    switch (uMsg)
    {
    case WM_DROPFILES:
    {
        ...
        std::string filePathStr(wFilePath.begin(), wFilePath.end());
        ...
        {
            std::cerr << "Program instance is null in HandleCustomWindowProc!" << std::endl;
            exit(1);
        }
        break;
    }
...
void Program::initButtons()
{
    ...
    // );
    //auto buttonCallback = std::make_shared<std::function<void()>>([this]() { this->ButtonPress(); });
    //auto buttonCallback2 = std::make_shared<std::function<void()>>([this]() { this->ButtonPress2(); });
...
{
    // image->create(1900, 1000, sf::Color(rand() % 256, rand() % 256, rand() % 256));
...
void Program::updatePixels()
{
    for (auto& pixel : activePixelVec) 
    {
        ...
        else if (m_image.getPixel(x + 1, y + 1) == sf::Color::Black || m_image.getPixel(x - 1, y + 1) == sf::Color::Black) {
            bool canFallLeft = (x > 0) && (m_image.getPixel(x - 1, y + 1) == sf::Color::Black);
            bool canFallRight = (x < static_cast<int>(m_image.getSize().x - 1)) && (m_image.getPixel(x + 1, y + 1) == sf::Color::Black);
...
{
    for (auto pixel = activePixelVec.begin(); pixel != activePixelVec.end();) {
